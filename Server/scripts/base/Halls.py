...
class Halls(KBEngine.Entity):
	...
	def reqRoomList(self,entityID):
		'''
		获取房间列表信息
		'''
		#DEBUG_MSG("Halls::reqRoomList, entity.%d,rooms:%s"% (entityID,str(self.rooms)))
		return self.rooms

	def createRoom(self,entityID,roomKey):
		'''
		创建房间
		'''
		roomDatas =  self.findRoom(entityID,roomKey,True)
		return roomDatas
	...
	def findRoom(self,entityID, roomKey, notFoundCreate = False):
		...
		"""
		DEBUG_MSG("Halls::findRoom:roomKey:%s,roomDatas:%s"%(roomKey,str(self.rooms)))
		...
		if not roomDatas:
			...
			roomDatas = {"roomEntityCall" : None, "PlayerCount": 0, "enterRoomReqs" : [], "roomKey" : self.lastNewRoomKey,"creater":entityID}
	...
	def enterRoom(self, entityCall, position, direction, roomKey):
		...
		请求进入某个Room中
		"""
		roomDatas = self.findRoom(entityCall.id, roomKey, False)

		if roomDatas == FIND_ROOM_NOT_FOUND:
			DEBUG_MSG("Halls::enterRoom, entity.%d,roomKey:%d not found !"%(entityCall.id,roomKey))
			return FIND_ROOM_NOT_FOUND
	...
	def leaveRoom(self, avatarID, roomKey):
		...
		"""
		roomDatas = self.findRoom(avatarID,roomKey, False)
